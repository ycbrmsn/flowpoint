...
ycbrmsn.getFlowPoint = (function () {
  ...
  FlowPoint.prototype = {
    ...
    changeRadius: function (point) {
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        if (points[i].imgWidth > flowPoint.maxImgWidth) { // 大于最大值
          ...
          if (flowPoint.radiusChange === 1 && points[i].radius <= flowPoint.initRadius - 1
            && points[i].imgWidth > flowPoint.initImgWidth - 1) {
            points[i].imgWidth = flowPoint.initImgWidth - 1;
          }
        }
      }
    ...
    animatePoint: function (point) {
      ...
      var p = points[0];
      flowPoint.paint();
      if (p.radius == flowPoint.initRadius && flowPoint.radiusChange == 1) {
